...
function ping (serverRegistration, timeout, callback, version) {
  switch (serverRegistration.data.type) {
    ...
    case 'PE':
      ...
      break
    case 'FiveM':
      var url = `http://${serverRegistration.data.ip}:${serverRegistration.data.port || 30120}`
      request({
        uri: `${url}/players.json`,
        method: 'GET',
        timeout: 5000
      }, (err, _, body) => {
        if (err) {
          logger.log('error', 'Failed to update FiveM services: %s', err.message)
          ...
          const payload = {
            players: {
              online: capPlayerCount(serverRegistration.data.ip, JSON.parse(body).length)
            }
          }
          request({
            uri: `${url}/info.json`,
            ...
            if (err) {
              ...
              payload.favicon = 'data:image/png;base64,' + JSON.parse(b).icon
              try {
                callback(null, payload)
              } catch (err) {
                logger.log('error', 'Failed to parse FiveM response: %s', err.message)
              }
            }
          })
        }
